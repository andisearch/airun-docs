...
See [Runtimes](/concepts/runtimes) for the complete compatibility matrix.
...
When no provider flag is given and no default is saved, `ai` operates in **passthrough mode** — a transparent wrapper that doesn't modify your Claude Code environment. Whatever auth method `claude` would use, `ai` uses too.

This means:
- If `ANTHROPIC_API_KEY` is in your environment → `ai` uses it (API billing)
- If no API key → `ai` uses your subscription (via `/login`)
- If `CLAUDE_CODE_USE_BEDROCK` is set → `ai` uses Bedrock

To override passthrough and force a specific provider:

| Goal | Command |
|------|---------|
| Force subscription | `ai --pro` or `ai --pro --set-default` |
| Force API key | `ai --apikey` |
| Force AWS Bedrock | `ai --aws` |
...
# Back to system default (matches your claude config)
ai
```
...
| 4. System default | No flags | Passthrough — matches native `claude` |
