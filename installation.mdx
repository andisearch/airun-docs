...
- `--opus` / `--high` - Highest-tier model (Opus 4.6, default)
- `--sonnet` / `--mid` - Mid-tier model (Sonnet 4.6)
...
`ai` with no flags matches your system `claude` configuration — same auth method, same model defaults. Provider flags (`--aws`, `--ollama`, etc.) only affect the current session:
