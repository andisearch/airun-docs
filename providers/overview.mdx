...
When no provider flag is given and no default is saved, AI Runner operates in **passthrough mode** — it doesn't load a provider or modify your environment. `claude` sees the same auth configuration as if run directly.

When a provider IS needed (explicit flag, saved default, or `DEFAULT_PROVIDER` in secrets.sh), auto-detection uses:
...
|------|---------|----------|---------------|
| **High** | `--opus`, `--high` | Complex reasoning, large refactors (default) | Claude Opus 4.6 |
| **Mid** | `--sonnet`, `--mid` | Cost-conscious coding tasks | Claude Sonnet 4.6 |
