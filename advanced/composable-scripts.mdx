...
```bash
ai task.md           # Auto-detects runtime (Claude Code)
ai --cc task.md      # Explicit: Claude Code
ai --codex task.md   # Explicit: Codex CLI
```

`--codex` shebangs work the same way as `--cc` shebangs for composable scripts — each script in a pipeline can target a different runtime.
