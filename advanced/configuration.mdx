...
## Pinning Opus to 4.6 (workaround for 4.7 regressions)

Some users have hit issues with Opus 4.7 since the 2026-04-16 release — most notably an
"invalid beta flag" error from AWS Bedrock that originates in Claude Code 2.1.111+
(see [anthropics/claude-code#30926](https://github.com/anthropics/claude-code/issues/30926)).
...
# Without this, the picker shows "Default ✓ Opus 4.7" even when ANTHROPIC_MODEL=4.6.
export ANTHROPIC_DEFAULT_OPUS_MODEL="claude-opus-4-6"
```

The `CLAUDE_MODEL_OPUS_*` vars set the active session's model (`ANTHROPIC_MODEL`).
...
without it, picking "Default" inside an interactive session jumps you back to 4.7.

Remove these lines (or revert to the defaults) once Anthropic ships a fix
upstream and you're ready to move back to 4.7.
...
```bash
export ANTHROPIC_MODEL="global.anthropic.claude-opus-4-7"             # Primary
...
- To get new model versions (e.g., Opus 4.7 → a newer Opus release)
...
# Uses: global.anthropic.claude-opus-4-7
