...
AI Runner's defaults track Anthropic's current Opus release (4.7 as of 2026-04-16).
You may prefer to pin to an older version — common reasons include personal
preference for a model's behavior, a known regression in your workload (e.g. some
users report worse long-context recall on 4.7 in very large codebases), or a
transient provider-side bug with the newest release. Pinning is a per-user choice
...
# Without this, the picker shows "Default ✓ Opus 4.8" even when ANTHROPIC_MODEL=4.6.
...
```bash
export ANTHROPIC_MODEL="global.anthropic.claude-opus-4-8"             # Primary
...
- To get new model versions (e.g., Opus 4.8 → a newer Opus release)
...
# Uses: global.anthropic.claude-opus-4-8
