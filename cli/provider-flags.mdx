...
<ParamField path="--pro" type="flag">
  ...
  **Note:** Use this to force your Claude subscription when `ANTHROPIC_API_KEY` is in your environment.
</ParamField>
...
|----------|--------|----------|
| 1. CLI flags | `ai --vertex task.md` | Explicit provider |
| 2. Shebang flags | `#!/usr/bin/env -S ai --aws` | Script-declared |
| 3. Saved defaults | `ai --aws --set-default` | Persistent choice |
| 4. System default | No flags | Passthrough — matches native `claude` |

With no flags and no saved default, `ai` is a transparent wrapper. Your environment is passed to `claude` unmodified.
